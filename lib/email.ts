...
// Draft replacement for the signup email, under review — not wired into
// the real signup flow yet. Kept separate from signupHtml/
// sendSignupConfirmationEmail so trying it out can never affect what new
// members actually receive. Fixes the one broken piece in the HTML Joanne
// exported from Resend's Broadcast editor: the unsubscribe link used a
// Resend Broadcast merge tag ({{{RESEND_UNSUBSCRIBE_URL}}}), which only
// resolves when sent through Resend's Broadcast tool — this app sends
// straight through the API, so that tag would go out unfilled. Swapped
// for the same per-user unsubscribe link every other email already uses.
export async function sendTestSignupEmail(to: string, userId: string) {
  ...
  await send(
    ...
    signupHtmlDraft(unsubscribeUrl),
    "Test signup email"
  );
}

function signupHtmlDraft(unsubscribeUrl: string) {
