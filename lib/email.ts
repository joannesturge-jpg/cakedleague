...
export async function sendTestLeagueCreatedEmail(to: string, userId: string) {
  const token = await createUnsubscribeToken(userId);
  const unsubscribeUrl = `${APP_URL}/unsubscribe?token=${token}`;
  await send(to, "[TEST] You created a league!", leagueCreatedHtmlDraft(unsubscribeUrl), "Test league created email");
}

// Exported from Resend's Template editor. Unlike the current live version,
// this draft doesn't mention the specific league by name and links to the
// dashboard/public leagues generally rather than the one just created — a
// deliberate simplification, not something lost in translation. The
// unsubscribe link is fixed the same way as the signup draft: swapped
// from a Resend Broadcast merge tag (only resolves when sent through
// Resend's Broadcast tool) to this app's own per-user link.
function leagueCreatedHtmlDraft(unsubscribeUrl: string) {
