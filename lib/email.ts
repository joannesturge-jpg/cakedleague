...
export async function sendLeagueJoinedEmail(to: string, leagueName: string, leagueId: string) {
  const safeLeagueName = escapeHtml(leagueName);
  const leagueUrl = `${APP_URL}/leagues/${leagueId}`;
  await send(to, `You're in: ${leagueName}`, leagueJoinedHtml(safeLeagueName, leagueUrl), "League joined confirmation");
}
...
export async function sendPicksDueReminderEmail(to: string, leagueName: string, dueLabel: string, leagueId: string) {
  ...
  const leagueUrl = `${APP_URL}/leagues/${leagueId}`;
  await send(to, `Picks due within 24 hours: ${leagueName}`, picksDueHtml(safeLeagueName, safeDueLabel, leagueUrl), "Picks due reminder");
}
...
function picksDueHtml(safeLeagueName: string, safeDueLabel: string, leagueUrl: string) {
