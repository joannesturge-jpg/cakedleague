...
import {
  sendTestSignupEmail,
  sendTestLeagueCreatedEmail,
  sendTestPicksDueEmail,
  sendTestLeagueJoinedEmail,
} from "@/lib/email";
...
// ?template=picks-due, or ?template=league-joined.
export async function GET(request: Request) {
  ...
  } else if (template === "league-joined") {
    await sendTestLeagueJoinedEmail(admin.email, admin.id);
  } else {
